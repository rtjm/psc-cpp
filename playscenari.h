...
{
	public:
		int rank, id, hold, fade;
		Sequence(int _rank, int _id, int _hold, int _fade)
		{
			rank = _rank;
			id = _id;
			hold = _hold;
...
class PlayScenari
{
	...
	public:
		...
		void GetNextStep()
		{
			if (loglevel > 0)
			{
				cout << "Define the next step for scenari " << scenari << endl;
			}
			...
			{ 
				...
				while(auto seqrow = mysql_fetch_row(seq))
				{
					...
					int _hold = stoi(seqrow[1]);
					int _fade = stoi(seqrow[2]);
		...
		void ComputeNextFrame()
		{
			...
			if (_activescenari)
			{
				...
				if (hold_counter != 0)	
				{
					...
					if (loglevel > 1)
					{
						cout << "Hold counter:" << hold_counter << endl;
					}
					new_frame.clear();
					// Playing frame
